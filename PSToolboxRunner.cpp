...
void PSToolboxRunner::Run(double t_max){
  ...
  while (t_global<t_max)
  {
   ...
   for(int i = 0; i < modifiers.size(); i++)
   {
    ...
    {
      cout << "\n--> Modifier applied at t = " << t_global << "s!\n";
      for (int j = 0; j < modifiers[i].connectors.size(); j++)
      {
        int idx = modifiers[i].connectors[j];
        double olddemand = cons[idx]->demand;
        double newdemand = modifiers[i].demands[j];
    ...
    */
    ...
    { 
      if (update_edges.at(i))
      {
		...
		//cout << "i = "<<i << "\t size of edges = "<< edges.size() << endl;
        edges.at(i)->UpdateInternal(t_next);

				if (DEBUG)
					cout<<"done."<<endl<<"\t\t Update LHS... ";

		//cout << "idx = "<<con_at_edge_start.at(i) << "\t size of cons = "<< cons.size() << endl;

        cons.at(con_at_edge_start.at(i))->Update(t_next,i);

				if (DEBUG)
					cout<<"done."<<endl<<"\t\t Update RHS... ";

		//cout << "idx = "<<con_at_edge_end.at(i) << "\t size of cons = "<< cons.size() << endl;
