...
void BioConnector::BioConnector_N_BioPipes(double t_target, int update_idx) {
  ...
  if (DEBUG) {
	...
	for (unsigned int i = 0; i < edges_idx.size(); i++) {
		...
		if ((is_front[i] && (edges[idx]->Get_dprop("v_conv")>0)) || (!is_front[i] && (edges[idx]->Get_dprop("v_conv")<0)))
        ...
        if (DEBUG){
        	for (unsigned int i = 0; i < edges_idx.size(); i++) {
        		int idx = edges_idx.at(i);
				cout<<endl<<"\t edge: "<<edges[idx]->Get_name()<<", is_front: "<<is_front[i]<<", v_conv: "<<edges[idx]->Get_dprop("v_conv")<<", is_inflow:"<<is_inflow[i];
        	}
		//cin.get();
          }
...
for (unsigned int i = 0; i < edges_idx.size(); i++){
  ...
  if (is_inflow.at(i)){
	...
	sum_mp+=fabs(mp);
        ...
        if (DEBUG)
    cout<<endl<<"\tinflow, edge"<<edges.at(idx)->Get_name()<<" C="<<C.transpose()<<", sum_C="<<sum_C.transpose()<<", sum_mp="<<sum_mp<<endl;
    //cin.get();
  }
  C=sum_C/sum_mp;
  }
	...
	if (type==0){
		...
		if (is_inflow){
			if (s_is_front)
				...
				s_edges->C.col(s_edges->Get_iprop("Npts")-1)=Cin;
		}
	}
}