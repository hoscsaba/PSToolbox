...
LWP::LWP(string _name,
         ...
         bool _save_data,
         bool _save_all_data,
         Gas *_gas) : PSToolboxBaseEdge("SCP", _name, _cspe_name, _cspv_name), Units() {
...
void LWP::Step(double dt_req) {
    ...
    UpdateInternalPoints(t + dt_req, false /* do_update_internal_time */, true);
    ...
    if (!left_boundary_points_already_updated) {
        ...
        cout << endl << "Name of the LWP pipe: " << name;
        cout << endl << endl << "LWP::Step() LEFT boundary was not yet updated";
        cout << endl << "This typically happens if the Connector class was NOT used earlier.";
        cout << endl <<
                "In such cases, use the LWP::Step(string,double,double,string,double,double,double) function instead,";
    ...
    if (!right_boundary_points_already_updated) {
        cout << endl << endl << "ERROR!!!";
        cout << endl << "Name of the LWP pipe: " << name;
        cout << endl << endl << "LWP::Step() RIGHT boundary was not yet updated";
        cout << endl << "This typically happens if the Connector class was NOT used earlier.";
        cout << endl <<
                "In such cases, use the LWP::Step(string,double,double,string,double,double,double) function instead,";
        cout << endl << "which updates the internal points AND the boundaries.";
...
void LWP::UpdateInternalPoints(double tnew, bool do_update_internal_time, bool is_called_from_Step_function) {
    dt = tnew - t;
    if (dt < 0) {
        printf("\n\n ERROR!!! LWP::UpdateInternalPoints(double, bool, bool)");
    ...
    // Needs to be done before timestep update
    if (is_BCRight_Wall) {
        BCRight(tnew, "Wall", 0, 0, true);
        right_boundary_points_already_updated = true;
    }
    if (is_BCLeft_Wall) {
        BCLeft(tnew, "Wall", 0, 0, true);
        left_boundary_points_already_updated = true;
    }

    // Timestep upate, if needed
    ...
    // NOT NEEDED, set these values to 'false' at the end of Step() function
    // Reset boundary conditions flag
    //left_boundary_points_already_updated=false;
    //right_boundary_points_already_updated=false;


    if (!is_called_from_Step_function) {
        if (save_data) {
            tmpvec.at(0) = t;
            tmpvec.at(1) = p(0);
            tmpvec.at(2) = p(Npts - 1);
...
void LWP::Pack(bool is_half_step) {
    double e, rhov, alpd = A * lambda / D / 2.;
    if (is_half_step) {
        for (unsigned int i = 0; i < Npts - 1; i++) {
            e = gas->Get_e_from_Tp(Thalf(i), phalf(i));
            rhov = rhohalf(i) * vhalf(i);
            Fhalf(i, 0) = rhov * A;
            Fhalf(i, 1) = (rhov * vhalf(i) + phalf(i)) * A;
            Fhalf(i, 2) = (rhov * e + phalf(i) * vhalf(i)) * A;

            Shalf(i, 0) = 0.;
            Shalf(i, 1) = -alpd * rhov * fabs(vhalf(i));
        ...
        for (unsigned int i = 0; i < Npts; i++) {
            ...
            F(i, 0) = rhov * A;
            F(i, 1) = (rhov * v(i) + p(i)) * A;
            F(i, 2) = (rhov * e + p(i) * v(i)) * A;

            S(i, 0) = 0.;
            S(i, 1) = -alpd * rhov * fabs(v(i));
...
bool LWP::BCLeft(double t_target, string type, double val1, double val2, bool write_to_node) {
    ...
    if (type == "StaticPres_and_StaticTemp_Inlet") {
        ...
        rhoBC = gas->Get_rho(pBC, TBC);
        double aBC = gas->Get_SonicVel(TBC, pBC);
        vBC = (pBC - beta_primitive) / rhoBC / aBC;
        BC_result_is_consistent = true;

        // cout << endl << "StaticPres_and_StaticTemp_Inlet";
        // cout << endl << "pBC   = " << pBC;
        ...
        if (vBC < 0) {
            ...
            if (write_to_node) {
                ...
                cout << endl << "\tLWP::BCLeft(): StaticPres_and_StaticTemp_Inlet -> negative velocity: " << vBC;
                cout << endl <<
                        "\tUsing Wall BC instead. If the situation persists, switch to other boundary condition";
                cout << endl;
            }
            vBC = 0.;
        ...
        // cout << endl << "LWP name: " << name;
        // cout << endl << "BCLeft::StaticPres_and_StaticTemp_Inlet " << name;
        // cout << endl << "p   BC  : " << pBC / 1.e5 << " bar";
        // cout << endl << "rho BC  : " << rhoBC << " kg/m3";
        // cout << endl << "T   BC  : " << TBC << " K";
        // cout << endl << "v   BC  : " << vBC << " m/s";
        // cout << endl << "beta   : " << beta_primitive << endl;
        // cin.get();
    ...
    if (type == "StaticPres_Outlet") {
        pBC = val1;
        rhoBC = GetC0AtFront(t_target);
        TBC = gas->Get_T(pBC, rhoBC);
        double aBC = gas->Get_SonicVel(TBC, pBC);
        vBC = (pBC - beta_primitive) / aBC / rhoBC;
        // cout << endl << "StaticPres_Outlet";
        // cout << endl << "pBC   = " << pBC;
        // cout << endl << "rhoBC = " << rhoBC;
        // cout << endl << "TBC   = " << TBC;
        // cout << endl << "aBC      = " << aBC;
...
bool LWP::BCRight(double t_target, string type, double val1, double val2, bool write_to_node) {
    double alpha = GetAlphaPrimitiveAtEnd(t_target);
    double vBC = 0., pBC = 0., rhoBC = 0., TBC = 0.;
    bool ok = false;
    ...
    if (type == "StaticPres_and_StaticTemp_Inlet") {
        ...
        if (vBC > 1.e-3) {
            ...
            if ((vBC > v_TOL) && (write_to_node)) {
                cout << endl << endl << " WARNING!! LWP pipe: " << name;
                cout << endl << "\tLWP::BCRight(): StaticPres_and_StaticTemp_Inlet -> positive velocity";
                cout << endl <<
                        "\tUsing Wall BC instead. If the situation persists, switch to other boundary condition";
                cout << endl;
            }
        ...
        // printf("\tStaticPres_and_StaticTemp_Inlet: vBC=%5.3e, pBC=%5.3e, TBC=%5.3e, rhoBC=%5.3e\n", vBC, pBC, TBC,
        //      rhoBC);
        //cin.get();
        ok = true;
    }
    ...
    if (type == "StaticPres_Outlet") {
        pBC = val1;
        rhoBC = GetC0AtEnd(t);
        //rhoBC = GetC0AtEnd(t + dt);
        TBC = gas->Get_T(pBC, rhoBC);
        double aBC = gas->Get_SonicVel(TBC, pBC);
        vBC = (alpha - pBC) / aBC / rhoBC;
        // cout << endl << "BCRight: StaticPres_Outlet() ";
        // cout << endl << "LWP name: " << name;
        // cout << endl << "p   BC  : " << pBC / 1.e5 << " bar = pBC";
        // cout << endl << "rho BC  : " << rhoBC << " kg/m3";
        // cout << endl << "T   BC  : " << TBC << " K";
        // cout << endl << "v   BC  : " << vBC << " m/s";
        ...
        if (vBC / aBC > 1.) {
            // Ideal gas only!
            // alpha= p + rho*a*c = R*T*rho + rho*k*R*T =R*T*rho*(1+k)
        ...
        ok = true;
        //printf("\tStaticPres_Outlet: vBC=%5.3e, pBC=%5.3e, TBC=%5.3e, rhoBC=%5.3e\n", vBC, pBC, TBC,
        //       rhoBC);
    }
...
void LWP::GetAllPrimitiveAtFront(double t_target, double &pR, double &vR, double &TR, double &rhoR) {
    ...
    if (delta_t < 0) {
        if (fabs(delta_t) < TOL)
            ...
            cout << endl
                    << "ERROR! LWP::GetBetaPrimitiveAtFront(), delta_t = " << delta_t << " < 0 ! (TOL=" << TOL << ")" <<
                    endl;
            cout << endl << "Name of pipe: " << name << endl;
            cin.get();
        }
...
double LWP::GetC0AtFront(double t_target) {
    ...
    if (delta_t < 0) {
        if (fabs(delta_t) < TOL)
            ...
            cout << endl
                    << "ERROR! LWP::GetC0AtFront(), t=" << t << ", delta_t = " << delta_t << " < 0 ! (TOL=" << TOL <<
                    ")" << endl;
            cout << endl << "Name of pipe: " << name << endl;
            cin.get();
        }
    ...
    if (delta_t > dt) {
        if (delta_t - TOL < dt)
            ...
            cout << endl
                    << "ERROR! LWP::GetC0AtFront(), t=" << t << ", delta_t = " << delta_t << " > dt= " << dt << endl;
            cout << endl << "Name of pipe: " << name << endl;
            cin.get();
        }
    }
    ...
    if (dxM < 0.) {
        ...
        if (DEBUG) {
            ...
            if (v(0) < 0) {
                ...
                cout << endl << "v(0)=" << v(0) << " < 0 ";
                cout << endl << "v(1)=" << v(1);
                cout << endl << "we should have v(0)< v1+dx/dt = " << v(1) << " + " << dx / delta_t << " = " << v(1) +
                        dx / delta_t;
            }
            if (v(0) > 0) {
                cout << endl << "dx=" << dx;
    ...
    if (dxM > dx) {
        ...
        if (DEBUG) {
            cout << endl
                    << "WARNING! LWP::GetC0AtFront(), dxM/dx = " << dxM / dx << " > 1 !!!" << endl;
            cout << endl << "Name of pipe: " << name << endl;
            //cin.get();
        }
    }
...
double LWP::GetC0AtEnd(double t_target) {
    ...
    if (delta_t < 0) {
        if (fabs(delta_t) < TOL)
            ...
            cout << endl << "Name of pipe: " << name << endl;
            cout << endl
                    << "WARNING! LWP::GetC0AtEnd(), t=" << t << ", delta_t = " << delta_t << " < 0  ! (TOL=" << TOL <<
                    ")" << endl;
            cout << endl << " t_pipe = " << t << ", t_target=" << t_target << endl;
            if (DEBUG)
                cin.get();
    ...
    if (delta_t > dt) {
        if (delta_t - TOL < dt)
            ...
            cout << endl
                    << "WARNING! LWP::GetC0AtEnd(), t=" << t << ", delta_t = " << delta_t << " > dt= " << dt << endl;
            cout << endl << "Name of pipe: " << name << endl;
            if (DEBUG)
                cin.get();
        }
    ...
    if (dxR < -dx / 1000.) {
        cout << endl
                << "WARNING! LWP::GetC0AtEnd() dxR = " << dxR << " < 0 !!!" << endl;
        cout << endl << "Name of pipe: " << name << endl;
        if (DEBUG)
            cin.get();
    }
    if (dxR > dx) {
        cout << endl
                << "WARNING! LWP::GetC0AtEnd() dxR/dx = " << dxR / dx << " > 1 !!!" << endl;
        cout << endl << "Name of pipe: " << name << endl;
        if (DEBUG)
            cin.get();
    }
...
void LWP::GetAllPrimitiveAtEnd(double t_target, double &pP, double &vP, double &TP, double &rhoP) {
    ...
    // cout << endl << "------------> LWP::GetAllPrimitiveAtEnd: t=" << t << ", t_target=" << t_target << ", delta_t=" <<
    //        delta_t << endl;
    ...
    if (delta_t < 0) {
        if (fabs(delta_t) < TOL)
            ...
            cout << endl << "Name of pipe: " << name << endl;
            cout << endl
                    << "ERROR! LWP::GetAllPrimitiveAtEnd(), delta_t = " << delta_t << " < 0  ! (TOL=" << TOL << ")" <<
                    endl;
            cout << endl << " t_pipe = " << t << ", t_target=" << t_target << endl;
            cin.get();
        }
...
double LWP::HeatFluxThroughWall(double Tgas, double v, double rho, double p, double &alpha_heat) {
    ...
    if (HEAT_TRANSFER_MODEL == "adiabatic") {
        ...
        cout << endl
                << "ERROR! LWP::HeatFluxThroughWall(), unknown heat transfer model: prop_string=" << HEAT_TRANSFER_MODEL
                << endl;
