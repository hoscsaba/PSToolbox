...
void Pump::H_fun_lin(double QQ, double n, double &a0, double &a1) {
	...
	if (isnan(QQ)) {
		...
		cout << endl << "\t\t p2 = p1 + a0 + a1*QQ = " << p1 << " + " << a0 * rho * g << " + " << a1 * rho * g <<
				" * Q (Pa)";
		cout << endl;
		cin.get();
	}
...
void Pump::GetEdgeEquationCoeffs(double t_target, bool is_front, double &LHS, double &coeff_Q, double &coeff_p1,
                                 double &coeff_p2) {
...
void Pump::Ini(double vini, double pstart, double dt_target) {
	Ini(1);
}
...
void Pump::Save_data() {
	...
	//cout<<endl<<"Pump "<<name<<" Save_data() called. Length of stored data: "<<data.size();
	//cin.get();
}
...
double Pump::Get_dprop(string prop_string) {
	double out = -1.;
	if (prop_string == "dt")
		out = dt;
	else if (prop_string == "n_nom")
		out = n_nom;
		...
		cout << endl
				<< "ERROR! Pump::Get_dprop(prop_string), unknown input: prop_string=" << prop_string << endl;

		cout << endl << "Name of pump: " << name << endl;
		cin.get();
	}
	return out;
};
