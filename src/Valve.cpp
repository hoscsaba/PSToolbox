...
void Valve::Update(double t_target, double pv, double pb, bool monitor_min_max) {
  ...
  if (debug) {
    double mp = Get_MassFlowRate(pv, 293., pb, 293., x);
    printf("\n t = %5.3e s, is_closed = %d, is_fully_open = %d, x=%5.3e mm, v=%+5.3e, dp=%5.3f bar, mp=%5.3e g/s",
           t, is_closed, is_fully_open, x * 1000., v, (pv - pb) / 1.e5, mp * 1000.);
    //cin.get();
  }
...
void Valve::rk45(
  ...
  //Fifth order approximation constants
  double d1 = 25. / 216., d2 = 0., d3 = 1408. / 2565., d4 = 2197. / 4104., d5 = -1. / 5., d6 = 0.;
  //Fourth order approximation constants
  ...
  while (!last_step) {
    ...
    while (retake_step) {
      ...
      if ((dx > 0) && (xx + dx > x_max)) {
        //we will reach the end in one step
        dx = x_max - xx;
        retake_step = false;
        last_step = true;
      }
      if ((dx < 0) && (xx + dx < x_max)) {
        //we have passed the maximum time
        dx = (x_max - xx);
        retake_step = false;
        last_step = true;
      }
      ...
      if (is_rk) {
        //This swithc is affected by changing the source code
        // RK45
        ...
        k6 = ValveODE(xx + c6 * dx, yy + a61 * dx * k1 + a62 * dx * k2 + a63 * dx * k3 + a64 * dx * k4 + a65 * dx * k5,
                      pars);
      ...
      if (debug) {
        //Display the errors in debug mode
      ...
      // Lepes beallitasa
      if (hiba > hiba_max) {
        //If the accuracy is not sufficient
        dx /= 2.; // a smaller timestep
        retake_step = true; // is used for retaking
        // printf("\n\t dx -> dx/2 (dx=%g)", dx);
        if (fabs(dx) < dx_min) {
          //Dispaly error message in case the necessary timestep is smaller than the minimum allowed one
          last_step = true;
...
double Valve::Get_MassFlowRate(double p_upstream, double T_upstream, double p_downstream, double T_downstream,
                               double xx) {
  double mp_;
  if (is_Gas)
    mp_ = Get_MassFlowRate_Compressible(p_upstream, T_upstream, p_downstream, T_downstream, xx);
  else
    mp_ = Get_MassFlowRate_InCompressible(p_upstream, p_downstream, ro, xx);
  mp = mp_; // mp is used only for storing lates mass flow rate for Get_dprop("mp");
  return mp_;
}

double Valve::Get_MassFlowRate_Compressible(double p_upstream, double T_upstream, double p_downstream,
                                            double T_downstream, double xx) {
  /*double dir_mul = 1.;
...
double Valve::Get_MassFlowRate_Compressible_UnChoked(double p_upstream, double T_upstream, double p_downstream,
                                                     double xx) {
  double rho_upstream = gas->Get_rho(p_upstream, T_upstream);
  double kappa = gas->Get_kappa_Tp();
...
string Valve::Info() {
  ...
  oss << "\n  cap. @ RT : " << mp_RT << " kg/s = " << mp_RT * kg_to_lbm <<
      "lbm/s, @ 110% of pset and xmax_restricted (pback=1.e5, Tin=293K)";
  oss << "\n sum(F) @ RT: " << SumStatForce(xmax, 1.e5 + 1.1 * p_set, 1.e5) <<
      " N, restoring force @ xmax_restricted, 1.1*pset";
