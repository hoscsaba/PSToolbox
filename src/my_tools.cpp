...
bool NR(VectorXd(*fun)(VectorXd & x, const VectorXd & pars),
        /**< [in] pointer to the function that the solution is searched for*/
...
) /**< [in] Show iterations? (not used, iterations always shown)*/
 {
...
double Input::extract_value(std::string name) {
	...
	while (i < elements && !over) {
		if (dataarray[i].name == name) {
			res = dataarray[i].value;
			over = true;
		}
		i++;
	}
	if (!over) {
		std::cout << name << " was not found in the system. 0 was passed as the value. Please check input" << std::endl;
	}
	return res;
}
...
void Input::parse_file(std::string filename) {
	...
	while (std::getline(infile, line)) {
		...
		if (splitplace2 ==
		    std::string::npos) { val = line.substr(splitplace1 + 1, line.length() - splitplace1); } else {
			val = val = line.substr(splitplace1 + 1, splitplace2 - splitplace1 - 1);
		}
