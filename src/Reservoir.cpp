...
VectorXd Reservoir::ReservoirODE(const double x, const VectorXd &y, const VectorXd &pars) {
  ...
  //printf("\n reservoir: p=%6.4f bar, a=%5.3e, vol=%5.3e, dq=%5.3e, dp/dt= %5.3f bar/s", y(0) / 1.e5, a, vol,
  //       pars(0) - pars(1), out(0) / 1.e5);
  //cin.get();
  return out;
}
  ...
  if ((prop_string == "Vres") || (prop_string == "V"))
    ...
    if (is_Gas)
      ...
      if (DEBUG) {
        ...
        cout << endl <<
            "This reservoir is ** NOT ** defined with liquid content and not gas -> no temperature data is available, returning 293K."
            << endl;
      }
      out = 293.;
    }
