...
Damper::Damper(const string _name,
                     const double _vol,
                     const double _a,
                     const double _rho,
                     const int _num_of_orifices,
                     const double _dia,
  ...
  beta = 0.;
  is_Gas = false;
  num_of_orifices = _num_of_orifices;
  dia = _dia;
...
void Damper::SetDebug(const bool _DEBUG) {
  DEBUG = _DEBUG;
}

string Damper::GetName() {
  return name;
}

void Damper::Ini(double pstart) {
  if (is_Gas) {
    cout << endl << "ERROR! Damper::Ini(pstart) was called but the fluid is IdealGas" << endl;
...
void Damper::Ini(double pstart, double Tstart) {
  if (!is_Gas) {
    cout << endl << "ERROR! Damper::Ini(pstart,Tstart) was called but the fluid is ** NOT ** IdealGas" << endl;
...
void Damper::UpdateDimlessPars(double pref, double mp_nevl, double omega) {
  if (is_Gas)
    a = gas->Get_SonicVel(293., 1.e5);
  beta = a * a * mp_nevl / vol / omega / pref;
}

void Damper::Update(double t_target, double p_connect) {
  //cout << endl << "mp_in=" << mp_in << ", mp_out=" << mp_out << endl;

  if (!Ini_done) {
    cout << endl << endl << "ERROR! Damper::Update() was called without initializing first";
  ...
  if (save_data) {
    ...
    tmpvec.at(0) = t;
    tmpvec.at(1) = p;
    tmpvec.at(2) = GetInternalPressureDrop();
    tmpvec.at(3) = mp;
    data.push_back(tmpvec);
  }
}

VectorXd Damper::DamperODE(const double x, const VectorXd &y, const VectorXd &pars) {
  VectorXd out(1);

  if (is_Gas){
    a = gas->Get_SonicVel(T, pars(0));
    rho=gas->Get_rho(pars(0),T);
  }
  mp=GetMassFlowRate(pars(0));
  out(0) = a * a / vol * mp;
  //printf("\n Damper: p_connect -> p=%6.4f -> %6.4f, T=%5.3e, rho=%5.3e, a=%5.3e, mp=%+5.3e, dp/dt= %5.3f bar/s",
  //       pars(0)/1.e5, y(0) / 1.e5, T, rho, a, mp,out(0) / 1.e5);

  return out;
}

double Damper::GetMassFlowRate(const double p_connect) {

  if (is_Gas)
    rho=gas->Get_rho( p_connect,T);

  double zeta=1.;
  double A= dia*dia*M_PI/4.;
  K = zeta*rho/2./(pow(rho*A,2.));
  double Kt=K/num_of_orifices/num_of_orifices;

  mp=signedSqrt((p_connect-p)/Kt);

  return mp;
  }

double Damper::GetInternalPressureDrop() {

  double zeta=1.;
  double Kt=2*pow(dia*dia*M_PI/4.,2.)*rho/zeta/num_of_orifices/num_of_orifices;
  return mp*fabs(mp)*Kt;
}
...
void Damper::rk45(
  ...
  while (!last_step) {
    ...
    while (retake_step) {
      ...
      if (is_rk) {
        // RK45
        k1 = DamperODE(xx, yy, pars);
        k2 = DamperODE(xx + c2 * dx, yy + a21 * dx * k1, pars);
        k3 = DamperODE(xx + c3 * dx, yy + a31 * dx * k1 + a32 * dx * k2, pars);
        k4 = DamperODE(xx + c4 * dx, yy + a41 * dx * k1 + a42 * dx * k2 + a43 * dx * k3, pars);
        k5 = DamperODE(xx + c5 * dx, yy + a51 * dx * k1 + a52 * dx * k2 + a53 * dx * k3 + a54 * dx * k4, pars);
        k6 = DamperODE(xx + c6 * dx,
                          yy + a61 * dx * k1 + a62 * dx * k2 + a63 * dx * k3 + a64 * dx * k4 + a65 * dx * k5, pars);
        ...
        k1 = DamperODE(xx, yy, pars);
        y1 = yy + dx * k1;
        y2 = yy + dx / 2 * (k1 + DamperODE(xx + dx, y1, pars));
      }
      ...
      if (hiba > hiba_max) {
        ...
        if (debug) //display succesful stepping
          printf(" ok.");
        if (hiba < hiba_max / 10.) {
          //if the error is really small, use a bigger timestep
          dx *= 2.;
          // printf("\n\t dx -> dx*2 (dxuj=%g)", dx);
        }
      }
...
void Damper::Save_data() {
  ...
  if (!save_data) {
    ...
    fprintf(pFile, "t (s); p (Pa); dp (Pa); mp (kg/s)\n");
...
double Damper::Get_dprop(string prop_string) {
  double out = 0.;
  if ((prop_string == "Vres") || (prop_string == "V"))
    ...
    if (is_Gas)
      ...
      if (DEBUG) {
        cout << endl << "WARNING!";
        cout << endl << "Name of Damper: " << name;
        cout << endl <<
            "This Damper is ** NOT ** defined with liquid content and not gas -> no temperature data is available, returning 293K."
            << endl;
      }
      out = 293.;
    }
    ...
    cout << endl
        << "ERROR! Damper::Get_dprop(prop_string), unknown input: prop_string=" << prop_string << endl
        << endl;
    cout << endl << "Possible options: Vres,V | a | T | p | rho | beta";
    cout << endl << "Name of Damper: " << name << endl;
    cin.get();
  }
  return out;
}

void Damper::Set_dprop(string prop_string, double val) {
  if (prop_string == "Vres")
    vol = val;
  else {
    cout << endl
        << "HIBA! Damper::Set_dprop(prop_string), ismeretlen bemenet: prop_string:" << prop_string << endl
        << endl;
  }
}
...
vector<double> Damper::Get_dvprop(string prop_string) {
  int Ntime = data.size();
...
double Damper::signedSqrt(double x) {
  double out=(x < 0. ? -1. : 1.) * sqrt(abs(x));
  return out;
}
